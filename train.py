...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(
            train_dataloader, leave=False, desc="Processing Train data"
        ):
        ...
        with torch.no_grad():
            for inputs, labels in tqdm(
                val_dataloader, leave=False, desc="Processing Val data"
            ):
                inputs = inputs.to(device)
                labels = labels.to(device)
...
def train_pgd_model(
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(train_dataloader, leave=False):
            ...
            if epoch > 0 or batch_cnt > 0:
                ...
                new_inputs = torch.zeros(
                    (batch_size + adv_batch_size, 3, input_shape[0], input_shape[1]),
                    dtype=torch.float64,
                )
        ...
        epoch_loss = running_loss / (
            ...
            + (round(len(train_dataloader.dataset) * ratio))
        )
...
def train_lca_model(
    ...
    if (
        model.LCA.track_metrics == True
        and metrics_path != None
        and metrics_plot_path != None
    ):
        # figures per batch
...
def train_pretrained_lca_model(
    model,
    input_shape,
    num_classes,
    batch_size,
...
def train_pretrained_lcafirstL_model(
    model,
    input_shape,
    num_classes,
    batch_size,
