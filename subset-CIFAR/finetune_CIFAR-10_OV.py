# -*- coding: utf-8 -*-
# This script uses pretrained ResNet18 weights (pretrained on ImageNet dataset)
# this script fine tunes it with CIFAR-10 OV dataset for 10 classes 78%
# %%
import random
import sys
...
# Model
model_name = "Fine-Tune CIFAR-10"                            # model architecture which shall be used
...
model_dir = stem_dir + "/models/_params/subset-CIFAR/finetune_CIFAR-10_OV/" + f"finetune_CIFAR-10_OV_lr_{lr}_bs_{batch_size}" + ".pt"
os.makedirs(os.path.dirname(model_dir), exist_ok=True)

fig_path_cm = stem_dir + "/figures/subset-CIFAR/finetune_CIFAR-10_OV/conf_matrix_" + f"pretrained_lr_{lr}_bs_{batch_size}" +  ".png"
fig_path_acc = stem_dir + "/figures/subset-CIFAR/finetune_CIFAR-10_OV/Acc_over_epochs_" + f"pretrained_lr_{lr}_bs_{batch_size}" + ".png"
fig_path_loss = stem_dir +"/figures/subset-CIFAR/finetune_CIFAR-10_OV/Loss_over_epochs_" + f"pretrained_lr_{lr}_bs_{batch_size}" + ".png"
fig_path_inference = stem_dir + "/figures/subset-CIFAR/finetune_CIFAR-10_OV/Inference_" +  f"pretrained_lr_{lr}_bs_{batch_size}" + ".png"
...
if __name__ == '__main__':
    ...
    """ Train model from pretrained weights on ImageNet """
    model,_ = initialize_model('resnet18', num_classes, feature_extract=False, weights=None, pretrained=True)
