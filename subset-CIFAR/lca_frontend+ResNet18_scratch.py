...
input_shape = (32,32)                               # size of image input as tuple
# Model
model_name = "LCA Frontend + ResNet18 From Scratch"                # model architecture which shall be used
...
# paths
model_dir = stem_dir + "/models/_params/subset-CIFAR/lca_frontend+ResNet18_scratch/" + f"_lr_{lr}_eta_{eta}_lambda_{LAMBDA}_ftr_{FEATURES}_size_{KERNEL_SIZE}_stride_{STRIDE}" + ".pt"
result_dir = stem_dir + "/models/_params/subset-CIFAR/lca_frontend+ResNet18_scratch/" + f"_lr_{lr}_eta_{eta}_lambda_{LAMBDA}_ftr_{FEATURES}_size_{KERNEL_SIZE}_stride_{STRIDE}"

fig_path = stem_dir + "/figures/subset-CIFAR/lca_frontend+ResNet18_scratch/"
fig_path_inference = stem_dir + "/figures/subset-CIFAR/lca_frontend+ResNet18_scratch/Inference_" +  f"_lr_{lr}_bs_{batch_size}" + ".png"
fig_path_acc = stem_dir + "/figures/subset-CIFAR/lca_frontend+ResNet18_scratch/Acc_over_epochs_lca_frontend" + f"_lr_{lr}_eta_{eta}_lambda_{LAMBDA}_ftr_{FEATURES}_size_{KERNEL_SIZE}_stride_{STRIDE}" + ".png"
fig_path_loss = stem_dir + "/figures/subset-CIFAR/lca_frontend+ResNet18_scratch/Loss_over_epochs" + f"_lr_{lr}_eta_{eta}_lambda_{LAMBDA}_ftr_{FEATURES}_size_{KERNEL_SIZE}_stride_{STRIDE}" + ".png"
...
""" Define model """
class LCA_Frontend_Scratch_CIFAR(nn.Module):
...
if __name__ == '__main__':
    ...
    model = LCA_Frontend_Scratch_CIFAR(lca_model=lca_pretrained, resnet18=resnet18, num_classes=num_classes)
    ...
    """ Train Model """
    _, train_acc_hist, val_acc_hist, train_loss_hist, val_loss_hist = train_pretrained_lca_model(model, input_shape=input_shape, num_classes=num_classes, batch_size=batch_size, device=device, train_dataloader=train_loader, val_dataloader=test_loader, 
                                                                                criterion=criterion, optimizer=optimizer, num_epochs_ResNet=num_epochs, LAMBDA=Lambda_train, scheduler=scheduler)                                                                                            
